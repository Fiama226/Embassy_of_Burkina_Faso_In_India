  ...
  return (
    <article className="bg-white rounded-2xl p-5 shadow-md border border-stone-100 hover:shadow-lg transition-shadow duration-300">
      ...
      <div className="flex justify-center mb-3">
        <div className="w-16 h-11 rounded-lg overflow-hidden shadow-sm ring-1 ring-stone-200 bg-white">
          ...
          <img
            ...
            decoding="async"
            onError={(e) => {
              console.error(`Failed to load flag: ${flagSrc}`, e);
              // Fallback: try alternate path
              const target = e.target as HTMLImageElement;
              if (target.src.includes('/flags/')) {
                target.src = flagSrc.replace('/flags/', '/');
              }
            }}
          />
        </div>
